...
public class SpringMailSenderService {
    ...
    private void sendEmailWithTemplateAndPdf(String to, String subject,
        ...
        try {
            ...
            log.info("Enviando e-mail com template '{}' e anexo '{}' para {}", templateName, attachmentName, to);
            mailSender.send(mimeMessage);
            log.info("E-mail enviado com sucesso para {}", to);

        } catch (MessagingException e) {
            log.error("Falha ao enviar e-mail (com PDF) para {}: {}", to, e.getMessage());
        }
    }
    ...
    private void recoverFromEmailError(Exception e,
                                      ...
                                      String templateName,
                                      Map<String, Object> variables) {
        log.error("Falha ao enviar e-mail (sem anexo) para {} após 3 tentativas: {}", to, e.getMessage());
    }
    ...
    private void recoverFromEmailWithPdfError(Exception e,
        ...
        log.error("Falha ao enviar e-mail (com PDF) para {} após 3 tentativas: {}", to, e.getMessage());
    }
